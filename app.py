...
def get_artesoes():
    ...
    if not artesao:
        ...
        logger.debug(f"%d Artesoes encontrados" % len(artesao))
...
@app.put('/artesao', tags=[artesao_tag],
            responses={"200": ArtesaoDelSchema, "404": ErrorSchema})
def put_artesao(query: ArtesaoBuscaSchema, form: ArtesaoSchemaPut):
    """Atualiza Artesao à base de dados
    ...
    session = Session()
    # fazendo a alteração
    artesao_exist = session.query(Artesao).filter(Artesao.box == artesao_box).one_or_none()

    if artesao_exist:
        update_artesao = artesao
        artesao_exist = update_artesao
        session.merge(artesao_exist)
        session.commit()
        return{"mesage": "Artesao alterado", "box": artesao_box}
    else:
        # se o Artesao não foi encontrado
        error_msg = "Artesao não encontrado na base :", artesao_box
        logger.warning(f"Erro ao alterar Artesao #'{artesao_box}', {error_msg}")
        return {"mesage": error_msg}, 404
